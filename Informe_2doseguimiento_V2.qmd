...
ggplot(datos_env) + 
  ...
  coord_polar(start = 5.49778266 ,direction = -1) +  # Transforma el gráfico en circular
  theme_minimal() +
  ...
  scale_x_continuous(limits = c(-180,180), breaks = c(-45,45,135,-135), 
                     labels = c("E","S","O","N"), minor_breaks = c(-90,0,90,180)) + # Ejes con direcciones cardinales
  theme(legend.position = "right") +
  scale_y_continuous(limits = c(380, 540), breaks = c(400,425,450,475,500,525,550)) + # Dejar un espacio en el centro
theme_light() +   scale_colour_manual(values = c("#fc8d59","#af8dc3","#d8b365","#1a9850"))
```

### **-Diferencias en las condiciones ambientales entre los tipos de vegetación**

La @fig-fig11 muestra el patrón de variación de variables topológicas, topográficas, cobertura y riqueza (a parte de la altura y la orientación) entre los diferentes tipos de vegetación. Evidenciando dos tipos de tendencia, una en la que los sitios con el tipo de vegetación **1**, que se concentran en mayor medida hacia el sur en alturas intermedias y altas, mantienen mayores valores de luz, temperatura, carga calorifica, climas más continentales y pendientes más inclinadas **(A; E; I; C; L)**. Teniendo diferencias aparentemente sustanciales con los tipos de vegetación **2 y 4** y posiblemente significativas con el tipo de vegetación **3** en algunas de estas variables (La falta de solapamiento de muescas no es suficiente para determinar diferencias significativas).

La otra tendencia se observa en las variables de humedad, nutrientes, reactividad. ph y profundidad del suelo **(B;D;F;G y J)**, que mantiene mayores valores en los sitios con tipo de vegetación **4** que se ubican en zonas de baja altitud, donde hay más sumideros e incidencia de sombra, favoreciendo el efecto de fenomeno de rivera, seguidos por sitios con el tipo de vegetación **3** que se ubican más hacia el norte donde la exposición a los rayos del sol es menor a lo largo del año, por este motivo el tipo de vegetación **3**, mantiene los menores valores de temperatura, luz, clima continental y carga calorifica.
...
-   <div>
    ...
    f7 = ggplot(datos_env,aes(x=factor(datos_env$Grupos),y= datos_env$CCSO)) + 
      geom_boxplot(aes(fill = factor(datos_env$Grupos)), notch = TRUE) + theme_classic() + 
      guides(fill = "none") +   scale_fill_manual(values = c("#fc8d59","#af8dc3","#d8b365","#1a9850")) +
    ...
    f12  = ggplot(datos_env,aes(x=factor(datos_env$Grupos),y= datos_env$CCSO)) + 
      geom_boxplot(aes(fill = factor(datos_env$Grupos)), notch = TRUE) + theme_classic() + 
      guides(fill = "none") +   scale_fill_manual(values = c("#fc8d59","#af8dc3","#d8b365","#1a9850")) +
...
g12 <- ggplot(datos_resum2, aes(x= Grupos , y=datos.sum, fill = Suelos)) + 
  ...
  scale_fill_manual(values = c("#d7191c","#fdae61","purple","#91cf60"))
...
g11 <- ggplot(datos_resum1, aes(x= reorder(Especies, -datos.sum) , y=datos.sum, fill = Grupos)) + 
     geom_bar(stat="identity", 
              position= "stack") +theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(x= "", y = expression("Cob. total"~(m^2)), fill = "tipos de vegetación")  +   scale_fill_manual(values = c("#fc8d59","#af8dc3","#d8b365","#1a9850"))
...
Los resultados del análisis de Escalamiento No Métrico Multidimensional (NMDS) en la @fig-fig17 , permite visualizar la influencia de variables ambientales sobre la composición de taxones de plantas en las parcelas del río Moldavia. Las elipses que rodean los puntos en el gráfico agrupan las parcelas según los cuatro tipos de vegetación identificados en el estudio. Cada flecha o vector representa una variable ambiental que podría influir en la composición de especies, donde las flechas azules corresponden a variables significativas, y las negras representan aquellas que no alcanzaron significancia estadística, según el ajuste de vectores realizado con la función `envfit`. Los nombres en texto rojo son los taxones de plantas presentes en las parcelas estudiadas.
...
El **tipo de vegetación 1** parece estar influenciado por variables ambientales como la temperatura, la luz, el clima continental y la carga calorífica del suelo. Este tipo de vegetación podría estar asociado a áreas con una mayor incidencia de luz solar, ya que, como se ha evidenciado en otros análisis exploratorios (@fig-fig11), tiende a estar ubicado en parcelas orientadas hacia el sur-suroeste. Estas áreas tienen una mayor exposición solar diaria, lo que podría explicar el predominio de especies adaptadas a estas condiciones ambientales.
