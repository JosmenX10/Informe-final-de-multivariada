---
title: "Efecto de las condiciones ambientales sobre la diversidad de plantas en los Valles profundos del río de Moldavia"
date: last-modified
...
¿Cual es el efecto de la condiciones ambientales sobre la composición de los ensamblajes de plantas en los valles profundos del río de Moldavia?

## **Objetivos**

-   Evaluar la relación entre las variables ambientales en los valles profundos del río Moldavia.

-   Determinar diferencias en las condiciones ambientales de las parcelas de acuerdo a los tipos de vegetación en los valles profundos del río Moldavia.
...
-   **Ho**: No existen relaciones entre las variables ambientales en los valles profundos del río Moldavia.

-   **Ha**: Existen relaciones entre las variables ambientales en los valles profundos del río Moldavia.
...
![](Base%20de%20datos%20y%20contextos/diagrama_de_flujo.svg){width="454"}
...
## **1.** Evaluar la relación entre las variables ambientales en los valles profundos del río Moldavia
...
ggsave("grafico.dispersion.svg",g1/g2, device = "svg", path = "C:\\Users\\Asus\\Desktop\\Analisis multivariado\\Informe-final-de-multivariada\\Graficos\\Explicaciones")

include_graphics("C:\\Users\\Asus\\Desktop\\Analisis multivariado\\Informe-final-de-multivariada\\Graficos\\Explicaciones\\grafico.dispersion2.svg")

```

A partir de la @fig-fig3**(A)** se evidencia un claro patron de los tipos de superficies o relieves a favor de la pendiente, a medida que cambia la altitud, pues se observan más parcelas con relieve concavo en altitudes bajas que en altitudes altas, así mismo los relieves neutros son más comunes en altitudes intermedias y los relieves convexos más comunes en altitudes mayores, aunque el leve solapamiento de las muescas dan la posibilidad de que no hubiera diferencias significativas en la altitud de los relieves neutros y convexos.

```{r,warning=FALSE,message=FALSE, fig.width=7,fig.height=4}
#| label: fig-fig3 
#| fig-cap: "A) Comparación de las altitudes entre distintos tipos de superficie en las parcelas según el eje a favor de la pendiente. B) Comparación de las altitudes para las parcelas en suelos líticos y no líticos. C) Comparación de las altitudes para las parcelas en suelos líticos y no líticos."
...
g3 = ggplot(datos_env,aes(x=factor(Superficie.PD),y=Elevación)) + 
  ...
  labs( x = "Tipos de relieve a lo largo de la pendiente", y = "Elevación (M.S.N.M)", tag = "a)")
...
g4 = ggplot(datos_env,aes(x=factor(Lítico),y=Elevación)) + 
  ...
  guides(fill = "none") +   scale_x_discrete(labels = c("1" = "Lítico", "0" = "No Litico")) + labs(x = "Condición del lítica del suelo", tag = "b)",y = "Elevación (M.S.N.M)")
...
g5 <- ggplot(frecuencia_factores) + geom_bar(aes(x = frecuencia_factores$Var1, y = frecuencia_factores$Freq, fill = frecuencia_factores$Var1), stat = "identity") + scale_fill_manual(values = rep(c("blue","orange"),3)) + guides(fill = "none") + theme_classic() + labs(x = "Tipos de suelo", y= "Frecuencia")

(g3 + g4)/g5 

```

También es posible observar cierto grado de diferencia en el tipo de suelo lítico y no lítico en diferentes altitudes, aunque podría no haber diferencias significativas en la @fig-fig3**(B)** se puede observar que en los diagramas de cajas bigotes la condición litica mantiene datos más concentrados a valores de mayor altitud que los no liticos.

La condición de suelo Lítico también es dependiente del tipo de relieve, siendo que en las parcelas de relieve convexo son más frecuentes que los suelos no liticos, y en las parcelas con relieve neutro ya los suelos líticos son menos frecuente, y en los relieves concavos las parcelas con suelos no líticos son mucho más frecuentes que los líticos. Por lo tanto, los relieves convexos pueden favorecer el asentamiento de suelos líticos antes que de suelos no líticos, mientras que en los concavos pasa lo contrario(@fig-fig3**(C)**).
...
#| label: fig-fig9 
#| fig-cap: "Comparación los indices de Hellinberg en distintas condiciones topográficas.A)Comparación del indice de humedad entre distintos tipos de superficie.B)Comparación del indice de nutrientes entre distintos tipos de superficie.C)Comparación del indice de humedad entre suelos líticos y no líticos."
...
datos_resum.s <- melt(datos_g1[,c(27:30,39)])
...
datos_resum1 <- datos_resum %>%        # Base de datos resumida
  ...
  summarise(datos.sum = sum(`Area de cobertura m^2`))   # Error estándar de cada grupo
...
g11 <- ggplot(datos_resum1, aes(x= reorder(Especies, -datos.sum) , y=datos.sum, fill = Grupos)) + 
     geom_bar(stat="identity", 
              position= "stack") +theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(x= "", y = expression("Cobertura total"~(m^2)), fill = "tipos de vegetación")
 
g12 <- ggplot(datos_resum2, aes(x= Grupos , y=datos.sum, fill = Suelos)) + 
     geom_bar(stat="identity", 
              position= "stack") +theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(x= "Tipos de vegetación", y = "Frecuencia", fill = "Condiciones de suelo")  + 
